...
def tasks():
	...
	cursor = g.db.execute('select name, due_date, prority, task_id from tasks where status=0')
	closed_tasks = [dict(dict(name=row[0], due_date=row[1], priority=row[2], task_id=row[3]) for row in cursor.fetchall())]
	g.db.close()
